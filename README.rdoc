...
* Implement has_many :through for swift-more so i can ditch the PostTag class
* Make the tags shit in blog/post.rb work properly.
...
dbicpp is a dependency for the swift rubygem we will be using, it's currenly
only listed in a third party lucid apt source, but should be fine for maverick
and saves installing a butt load of dev packages so we can compile from source.

* Dependencies for swift gem
    sudo vim /etc/apt/sources.list
    # Add deb http://ppa.launchpad.net/deepfryed/ppa/ubuntu lucid main
    # Add deb-src http://ppa.launchpad.net/deepfryed/ppa/ubuntu lucid main
    sudo apt-key adv --recv-keys --keyserver keyserver.ubuntu.com C694F4225CBEFE90
    sudo apt-get update
    sudo apt-get install dbic++-pg dbic++-dev build-essentials libpcre3-dev uuid-dev
